...
class Kxpay extends PaymentModule {
	...
	public function __construct() {
		...
		switch ($this->env) {
			case 1:
				$this->urltoken = 'https://tpvvirtual-test.universalpay.es/client2/token-pro';
				$this->urlenv = 'https://tpvvirtual-test.universalpay.es/client2/load';
				$this->urlstatus = 'https://tpvvirtual-test.universalpay.es/client2/rservices/checkOperation';
				break;
			case 2: 
				$this->urltoken = 'https://tpvvirtual.universalpay.es/client2/token-pro';
				$this->urlenv = 'https://tpvvirtual.universalpay.es/client2/load';
				$this->urlstatus = 'https://tpvvirtual.universalpay.es/client2/rservices/checkOperation';
				break;
		}
	...
	private function _postValidation() {
		if (Tools::isSubmit ('btnSubmit')) {
			if (!Tools::getValue ('merchant_identifier') || !checkAlphanumeric (Tools::getValue ('merchant_identifier')))
				$this->post_errors [] = $this->l('El identificador proporcionado por UniversalPay es obligatorio.');
			if (!Tools::getValue ('secure_key') || !checkAlphanumeric (Tools::getValue ('secure_key')))
				$this->post_errors[] = $this->l('La clave de seguridad proporcionada por UniversalPay es obligatoria.');
	...
	private function _displayPayment(){
		...
		.'<h1 style="text-align: center;">'.$this->l('Este módulo le permite aceptar pagos mediante Tarjeta.').'</h1></div>';
	}
	...
	private function createParameter($parameters){
		...
		$data['LOCALE'] = Tools::substr ($this->context->language->locale, 0, 2);
		...
		$params['IDENTIFIER'] = $address->dni;
		$params['PERSONAL_IDENTITY_NUMBER'] = $customer->id;
...
/* version presteamshop*/
    public function hookPaymentOptions($params)
    {
        ...
        $option->setModuleName($this->name)
            ->setCallToActionText($this->l('Pago con tarjeta'))
